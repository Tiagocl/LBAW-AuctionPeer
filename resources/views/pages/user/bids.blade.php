        ...
        <div class="grid grid-cols-3 gap-4">
            @foreach ($bids as $bid)
                ...
                <div class="bg-gray-100 p-4 rounded shadow user-auction-lists">
                    ...
                    <h3 class="mt-2 text-3xl font-semibold">{{ $bid->auction->title }}</h3>
                    <p class="text-xl text-white">Your bid: ${{ $bid->amount }}</p>
                    <p class="text-xl text-white">Current highest bid: ${{ $bid->auction->current_bid }}</p>
                </div>
                </a>
            @endforeach
        </div>
